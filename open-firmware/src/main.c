...
{
    /* OEM: GPIOC IDR bits[4:0] active-low; bit5 is virtual (UP+DOWN+LIGHT). */
    uint8_t idr = (uint8_t)(hw_gpio_read_idr('C') & OEM_BTN_MASK);
    uint8_t raw = (uint8_t)(idr | OEM_BTN_VIRTUAL);
    if ((idr & (OEM_BTN_UP | OEM_BTN_DOWN | OEM_BTN_LIGHT)) == 0u)
