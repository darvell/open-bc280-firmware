...
def _dump_env_once():
    if state.get("env_dumped"):
        return
    state["env_dumped"] = True
    try:
        if not os.path.isdir(OUTDIR):
            os.makedirs(OUTDIR)
        with open(os.path.join(OUTDIR, "dma1_env.txt"), "w") as f:
            try:
                f.write("self_type=%s\n" % type(self))
                f.write("self_has_Machine=%s\n" % hasattr(self, "Machine"))
                f.write("self_dir=%s\n" % [x for x in dir(self) if not x.startswith("_")][:80])
            except Exception as e:
                f.write("self_inspect_error=%s\n" % e)
            try:
                f.write("request_type=%s\n" % type(request))
                f.write("request_dir=%s\n" % [x for x in dir(request) if not x.startswith("_")][:120])
            except Exception as e:
                f.write("request_inspect_error=%s\n" % e)
    except Exception:
        pass
...
def _do_transfer(ch_idx):
    ...
    # Defer UART/SPI RX DMA channels: they are driven by peripheral request lines (RXNE / SPI RX)
    # and must only advance when the peripheral produces bytes/words.
    #
    # We currently drive these from peripheral/bridge scripts:
    #   - SPI1 RX via paired SPI1 TX channel (see special-case below)
    #   - USART1 RX via BLE passthrough (TTM + 0x55 frames)
    #   - USART2 RX via motor bus frames
    if not direction_mem_to_periph and paddr in (0x4001300C, 0x4000380C, 0x40013804, 0x40004404):
    ...
    if direction_mem_to_periph and paddr in (0x4001300C, 0x4000380C):
        # Optional VRAM snapshot for LCD DMA bursts.
        try:
            if not state.get("vram_dumped") and count > 1000 and 0x60020000 <= maddr < 0x60040000:
                data = bus.ReadBytes(maddr, min(count * unit, 0x20000))
                if not os.path.isdir(OUTDIR):
                    os.makedirs(OUTDIR)
                with open(os.path.join(OUTDIR, "vram_dump.bin"), "wb") as f:
                    f.write(bytearray(data))
                state["vram_dumped"] = True
                _log("VRAM dump captured len=%d" % len(data))
        except Exception:
            pass
