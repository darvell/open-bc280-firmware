...
- Dense, consistent table layout:
  - Raw inputs (brake, throttle raw/filtered, cadence raw, speed pulses)
  - Link stats (errors/retries)
  - State machine states (assist mode, walk, cruise)
...
- `open-firmware/ui/gfx.[ch]` — low-level primitives + LCD window streaming
- `open-firmware/ui/font_stroke.[ch]` — stroke glyphs for small text
- `open-firmware/ui/font_digits.[ch]` — segment definitions for big digits
- `open-firmware/ui/icons.[ch]` — procedural icon set
- `open-firmware/ui/widgets_*.c` — widgets (speed chip, battery, cards)
- `open-firmware/ui/screens_*.c` — per-screen layout + event handling
- `open-firmware/ui/ui.[ch]` — state machine + dirty rect scheduler
