 ...
 *        bit 3: brake_flag
 *        bit 2: speed_over_limit
 *        bit 1: walk_mode
 *        bit 0: motor_running_flag
 */
static void handle_0x52_request(sim_dwg_motor_t *m, const uint8_t *payload, uint8_t len)
{
    if (len >= 2)
    {
        ...
        m->config.lights_enabled = (payload[1] >> 6) & 1;
        m->walk_assist_active = (payload[1] >> 5) & 1;
        /* bit 4: cruise_control */
        m->config.brake_flag = (payload[1] >> 3) & 1;
        m->speed_over_limit = (payload[1] >> 2) & 1;
        /* bit 1: walk_mode */
        /* bit 0: motor_running_flag */
