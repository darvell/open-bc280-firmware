#!/usr/bin/env python3
"""Renode test for MENU-only long press (should NOT reboot)."""
...
def main() -> int:
    ...
    try:
        ...
        client.set_state(rpm=0, torque=0, speed_dmph=0, soc=80, err=0, buttons=BUTTON_MENU)
        time.sleep(LONG_PRESS_S)
        client.set_state(rpm=0, torque=0, speed_dmph=0, soc=80, err=0, buttons=BUTTON_MENU)
        print("  [OK] Sent MENU long press")
        ...
        expect(ms_after >= ms_before, "Unexpected reboot after MENU long press")

        print("PASS: menu-only long press does not reboot")
        return 0
