...
def build_header(build_id: int, payload: bytes, crc_override: int) -> bytes:
...
def main() -> int:
    ...
    try:
        ...
        bad_crc = good_crc ^ 0xFFFFFFFF
        header = build_header(build_id, payload, bad_crc)
        ...
        expect(st2.active_slot == active, "active slot changed despite corrupted image")
        expect(st2.build_id == st.build_id, "build_id changed despite corrupted image")
        expect(st2.pending_slot == 0xFF, "pending slot not cleared after rollback")

        print("PASS: A/B rollback on corrupted image")
        return 0
