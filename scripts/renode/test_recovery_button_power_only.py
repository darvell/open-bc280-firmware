#!/usr/bin/env python3
"""Renode test for POWER-only long press (should NOT reboot)."""
...
def main() -> int:
    ...
    try:
        ...
        print(f"  [OK] Uptime before long press: {ms_before} ms")

        client.set_state(rpm=0, torque=0, speed_dmph=0, soc=80, err=0, buttons=BUTTON_POWER)
        time.sleep(LONG_PRESS_S)
        client.set_state(rpm=0, torque=0, speed_dmph=0, soc=80, err=0, buttons=BUTTON_POWER)
        print("  [OK] Sent POWER long press")
        ...
        print(f"  [OK] Uptime after long press: {ms_after} ms")
        expect(ms_after >= ms_before, "Unexpected reboot after POWER long press")

        print("PASS: power-only long press does not reboot")
        return 0
