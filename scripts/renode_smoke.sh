...
# 3) asserts UART1 prints "[open-fw] boot" and at least one "[open-fw] t=" line

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
RENODE_RESC="$ROOT_DIR/open-firmware/renode/bc280_open_firmware.resc"
...
echo "[smoke] building open-firmware (make -C open-firmware)" >&2
make -C "$ROOT_DIR/open-firmware" -s
...
if [[ "$RENODE_MODE" == "mono" ]]; then
    ...
    BC280_RENODE_OUTDIR="$OUTDIR" BC280_LCD_OUTDIR="$OUTDIR" "$MONO_BIN" "$RENODE_BIN" --disable-xwt \
        -e "include @$RENODE_RESC" \
        -e "emulation RunFor \"${SIM_SECONDS}s\"" \
        -e "quit" \
        >"$RENODE_LOG" 2>&1 &
else
    BC280_RENODE_OUTDIR="$OUTDIR" BC280_LCD_OUTDIR="$OUTDIR" "$RENODE_BIN" --disable-xwt \
        -e "include @$RENODE_RESC" \
        -e "emulation RunFor \"${SIM_SECONDS}s\"" \
        -e "quit" \
        >"$RENODE_LOG" 2>&1 &
...
while (( elapsed < TIMEOUT_SECONDS )); do
    if [[ -f "$UART_LOG" ]]; then
        if grep -Fq "[open-fw] boot" "$UART_LOG"; then
            found_boot=1
        fi
        if grep -Fq "[open-fw] t=" "$UART_LOG"; then
            found_tick=1
        fi
        if (( found_boot && found_tick )); then
            break
        fi
    fi
    sleep 1
...
echo "[smoke] PASS: observed boot + status lines in ${elapsed}s" >&2
exit 0
