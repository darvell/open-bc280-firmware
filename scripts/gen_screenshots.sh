...
# Outputs PNG files to docs/firmware/examples/
#
# Requirements:
...
OUT_DIR="$REPO_ROOT/docs/firmware/examples"
LCD_DIR="$REPO_ROOT/out/lcd_out"
...
if [ ! -f "$BUILD_DIR/build.ninja" ]; then
    ...
    meson setup "$BUILD_DIR" "$REPO_ROOT" --buildtype=debug
fi

# Build host simulator
echo "Building host_sim..."
ninja -C "$BUILD_DIR" host_sim
...
HOST_SIM="$BUILD_DIR/host_sim"
if [ ! -x "$HOST_SIM" ]; then
    HOST_SIM="$BUILD_DIR/tests/host/host_sim"
fi
if [ ! -x "$HOST_SIM" ]; then
    echo "Error: host_sim not found at:"
    echo "  - $BUILD_DIR/host_sim"
    echo "  - $BUILD_DIR/tests/host/host_sim"
    exit 1
fi
