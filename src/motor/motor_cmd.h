 ...
 *
 * Args:
 *   level - 1-based assist level index (0 is treated as 1)
 *
 * Note: Updates g_active_vgear and queues a 0x52 command at the
 *       next ISR TX interval. Uses shengyi_assist_level_mapped()
 *       to convert to OEM assist bytes.
 */
void motor_cmd_set_assist(uint8_t level);

/*
 * Set OEM assist gear count
 *
 * Args:
 *   count - desired count (OEM supports 1, 3, 5, 6, 9)
 *
 * Returns:
 *   Applied count (clamped/mapped to closest OEM-supported count).
 *
 * Note: This updates the virtual gear table count and regenerates
 *       per-gear scales using current min/max/shape. It also clamps
 *       the active gear and queues a fresh 0x52 command.
 */
 ...
 *
 * Args:
 *   idx - 1-based gear index (0 is treated as 1)
 */
