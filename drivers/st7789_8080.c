...
static void st7789_write_data16(const st7789_8080_bus_t *bus, uint16_t data)
{
    if (bus->write_data16) {
        bus->write_data16(data);
        return;
    }

    /* 8-bit fallback: send MSB then LSB. */
    st7789_write_data(bus, (uint8_t)(data >> 8));
    st7789_write_data(bus, (uint8_t)(data & 0xFFu));
}
                              ...
                              uint32_t count)
{
    ...
    for (uint32_t i = 0; i < count; ++i)
        st7789_write_data16(bus, pixels[i]);
}
                           ...
                           uint16_t color)
{
    ...
    st7789_8080_set_address_window(bus, x, y,
                                   (uint16_t)(x + w - 1u),
                                   (uint16_t)(y + h - 1u));

    uint32_t count = (uint32_t)w * (uint32_t)h;
    for (uint32_t i = 0; i < count; ++i)
        st7789_write_data16(bus, color);
}
...
void st7789_8080_init_oem(const st7789_8080_bus_t *bus)
{
    ...
    st7789_delay_ms(bus, 20u);  /* Allow display to stabilize after DISPON */
}
