...
```bash
scripts/preflight_open_firmware.py --image build/open_firmware.bin
```
...
ninja -C build-host host_sim
./build-host/tests/host/host_sim
open out/lcd_out/host_lcd_latest.ppm
```
...
```
Outputs go to `docs/firmware/examples/` (tracked in git) with 2× scaled PNGs for each page.
The script builds `host_sim`, forces each page in sequence, and converts PPM → PNG.

**UI Gallery** — see [examples/](examples/) for current screenshots:
...
- Recovery entry flows (button combo) must use the same bootloader-flag path and never bypass the OEM bootloader.
...
## Python library (bc280fw)
For a more complete client (typed models, streaming helpers, config/log parsing),
use the packaged library under `python/bc280fw/`:
...
```python
from bc280fw import BC280Client, TelemetryV1

with BC280Client.serial('/tmp/uart1', baud=9600, timeout=0.2) as client:
    client.ping()
    client.set_stream_period_ms(500)
    for frame in client.iter_frames(timeout=2.0):
        if isinstance(frame, TelemetryV1):
            print(frame)
            break
```
