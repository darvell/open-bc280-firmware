...
static void test_comm_frame_build_validate(void)
{
    uint8_t out[16];
    uint8_t payload[2] = {0xAAu, 0x55u};
    size_t len = comm_frame_build(out, sizeof(out), 0x10u, payload, sizeof(payload));
    assert_eq_u16((uint16_t)len, 6u, "frame build len");
    assert_eq_u8(out[0], COMM_SOF, "frame build sof");
    assert_eq_u8(out[1], 0x10u, "frame build cmd");
    assert_eq_u8(out[2], 0x02u, "frame build payload len");
    assert_eq_u8(out[3], 0xAAu, "frame build payload[0]");
    assert_eq_u8(out[4], 0x55u, "frame build payload[1]");
    assert_eq_i32(comm_frame_validate(out, len, NULL), 1, "frame build validate");

    uint8_t empty[8];
    len = comm_frame_build(empty, sizeof(empty), 0x01u, NULL, 0);
    assert_eq_u16((uint16_t)len, 4u, "frame build empty len");
    assert_eq_u8(empty[0], COMM_SOF, "frame empty sof");
    assert_eq_u8(empty[1], 0x01u, "frame empty cmd");
    assert_eq_u8(empty[2], 0x00u, "frame empty payload len");
    assert_eq_i32(comm_frame_validate(empty, len, NULL), 1, "frame empty validate");

    len = comm_frame_build(out, 4, 0x10u, payload, sizeof(payload));
    assert_eq_u16((uint16_t)len, 0u, "frame build cap too small");

    len = comm_frame_build(out, sizeof(out), 0x10u, NULL, 1);
    assert_eq_u16((uint16_t)len, 0u, "frame build null payload");

    assert_eq_i32(comm_frame_validate(out, 5, NULL), 0, "frame validate length mismatch");
    out[0] = 0x00u;
    assert_eq_i32(comm_frame_validate(out, 6, NULL), 0, "frame validate bad sof");
}

static void test_comm_parser_feed(void)
{
    uint8_t out[8];
    uint8_t payload[1] = {0x42u};
    size_t len = comm_frame_build(out, sizeof(out), 0x22u, payload, sizeof(payload));
    uint8_t buf[8] = {0};
    uint8_t parsed_len = 0;
    uint8_t len_io = 0;
    comm_parse_result_t res = COMM_PARSE_NONE;

    for (size_t i = 0; i < len; ++i)
    {
        res = comm_parser_feed(buf, sizeof(buf), COMM_MAX_PAYLOAD, &len_io, out[i], &parsed_len);
        if (i + 1u < len)
            assert_eq_i32(res, COMM_PARSE_NONE, "parser feed mid-frame");
    }
    assert_eq_i32(res, COMM_PARSE_FRAME, "parser feed frame");
    assert_eq_u8(parsed_len, (uint8_t)len, "parser feed len");
    assert_eq_u8(len_io, 0u, "parser feed reset");
}

static void test_comm_parser_oversize(void)
{
    uint8_t buf[8] = {0};
    uint8_t len_io = 0;
    uint8_t parsed_len = 0;
    ...
    res = comm_parser_feed(buf, sizeof(buf), 1u, &len_io, COMM_SOF, &parsed_len);
    assert_eq_i32(res, COMM_PARSE_NONE, "parser oversize sof");
    res = comm_parser_feed(buf, sizeof(buf), 1u, &len_io, 0x10u, &parsed_len);
    assert_eq_i32(res, COMM_PARSE_NONE, "parser oversize cmd");
    res = comm_parser_feed(buf, sizeof(buf), 1u, &len_io, 0x05u, &parsed_len);
    assert_eq_i32(res, COMM_PARSE_ERROR, "parser oversize error");
    assert_eq_u8(len_io, 0u, "parser oversize reset");
}

static void test_comm_parser_ignores_noise(void)
{
    uint8_t buf[8] = {0};
    uint8_t len_io = 0;
    uint8_t parsed_len = 0;
    ...
    res = comm_parser_feed(buf, sizeof(buf), COMM_MAX_PAYLOAD, &len_io, 0x00u, &parsed_len);
    assert_eq_i32(res, COMM_PARSE_NONE, "parser noise byte");
    assert_eq_u8(len_io, 0u, "parser noise len_io");
}
...
int main(void)
{
    ...
    test_comm_frame_build_validate();
    test_comm_parser_feed();
    test_comm_parser_oversize();
    test_comm_parser_ignores_noise();
    test_clamp_helpers();
