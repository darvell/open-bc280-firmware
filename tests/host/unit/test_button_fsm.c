...
/*
 * Test: MENU+POWER combo
 */
TEST(fsm_combo_menu_power)
{
    button_fsm_t fsm;
    button_fsm_init(&fsm);
    ...
    button_fsm_update(&fsm, BTN_MASK_MENU | BTN_MASK_POWER, now);
    now += 800;
    button_fsm_update(&fsm, BTN_MASK_MENU | BTN_MASK_POWER, now);

    event_t evt;
    ASSERT_TRUE(button_fsm_poll_event(&fsm, &evt));
    ASSERT_EVENT_TYPE(evt, EVT_BTN_COMBO_MENU_POWER);
}

TEST(fsm_combo_menu_power_short)
{
    button_fsm_t fsm;
    button_fsm_init(&fsm);
...
TEST(combo_menu_power_maps_long_press)
{
    button_track_t track;
    button_track_reset_state(&track);

    uint8_t short_press = 0;
    uint8_t long_press = 0;
    uint32_t now = 1000;

    button_track_update_state(&track, UI_PAGE_BUTTON_RAW | UI_PAGE_BUTTON_POWER,
                              0xFFu, now, 0u, &short_press, &long_press);
    ASSERT_TRUE(short_press == 0);
    ASSERT_TRUE(long_press == 0);

    now += 800;
    button_track_update_state(&track, UI_PAGE_BUTTON_RAW | UI_PAGE_BUTTON_POWER,
                              0xFFu, now, 0u, &short_press, &long_press);
    ASSERT_TRUE((long_press & (UI_PAGE_BUTTON_RAW | UI_PAGE_BUTTON_POWER)) ==
                (UI_PAGE_BUTTON_RAW | UI_PAGE_BUTTON_POWER));
}

TEST(combo_menu_power_maps_short_press)
{
    button_track_t track;
    button_track_reset_state(&track);
    ...
    now += 500;
    button_track_update_state(&track, 0u, 0xFFu, now, 0u, &short_press, &long_press);
    ASSERT_TRUE((short_press & (UI_PAGE_BUTTON_RAW | UI_PAGE_BUTTON_POWER)) ==
                (UI_PAGE_BUTTON_RAW | UI_PAGE_BUTTON_POWER));
    ASSERT_TRUE(long_press == 0);
}
...
int main(void)
{
    ...
    RUN_TEST(fsm_combo_menu_power);
    RUN_TEST(fsm_combo_menu_power_short);
    RUN_TEST(combo_menu_power_maps_long_press);
    RUN_TEST(combo_menu_power_maps_short_press);
